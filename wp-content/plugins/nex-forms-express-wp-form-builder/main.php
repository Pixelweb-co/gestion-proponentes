...
function NEXForms_test_page(){
	...
	global $wpdb;
			...
			$c_logic = $wpdb->get_var('SELECT conditional_logic_array FROM '.$wpdb->prefix.'wap_nex_forms WHERE Id=209'.$form_Id);
			...
			foreach($rules as $rule)
				{
				if(count(get_object_vars($rule->conditions))==1 && count(get_object_vars($rule->actions))==1)
					{
