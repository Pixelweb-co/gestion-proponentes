...
function ws_force_login_upgrade_completed( $upgrader_object, $options ) {
        $our_plugin = plugin_basename( __FILE__ );
        if( $options['action'] == 'update' && $options['type'] == 'plugin' && isset( $options['plugins'] ) ) {

                foreach( $options['plugins'] as $plugin ) {
                        if( $plugin == $our_plugin ) {
                                update_option( 'wsforce-login-active-option', '1' );
                        }
                }
        }
}
add_action( 'upgrader_process_complete', 'ws_force_login_upgrade_completed', 10, 2 );
